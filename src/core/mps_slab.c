...
mps_slab_init(mps_slab_pool_t *pool, u_char *addr, size_t pool_size)
{
    ...
    pool->stats = (mps_slab_stat_t *) p;
    ngx_memzero(pool->stats, n * sizeof(mps_slab_stat_t));
...
mps_slab_alloc_locked(mps_slab_pool_t *pool, size_t size)
{
    ...
    mps_slab_page_t  *page, *prev, *slots;
    ...
    if (page) {
        if (shift < mps_slab_exact_shift) {
            ...
            pool->stats[slot].total += (mps_pagesize >> shift) - n;
            ...
            pool->stats[slot].total += mps_pagesize >> shift;
...
mps_slab_free_locked(mps_slab_pool_t *pool, void *p)
{
    ...
    n = ((u_char *) p - pool->start) >> mps_pagesize_shift;
    page = &pool->pages[n];
    slab = page->slab;
    ...
    case MPS_SLAB_SMALL:
        ...
        if (bitmap[n] & m) {
            ...
            if (page->next == NULL) {
                ...
                page->prev = (uintptr_t) &slots[slot] | MPS_SLAB_SMALL;
                page->next->prev = (uintptr_t) page | MPS_SLAB_SMALL;
            }
    ...
    case MPS_SLAB_EXACT:
        ...
        if (slab & m) {
            ...
            if (slab == MPS_SLAB_BUSY) {
                ...
                page->prev = (uintptr_t) &slots[slot] | MPS_SLAB_EXACT;
                page->next->prev = (uintptr_t) page | MPS_SLAB_EXACT;
            }
    ...
    case MPS_SLAB_BIG:
        ...
        if (slab & m) {
            ...
            if (page->next == NULL) {
                ...
                page->prev = (uintptr_t) &slots[slot] | MPS_SLAB_BIG;
                page->next->prev = (uintptr_t) page | MPS_SLAB_BIG;
            }
